		...
		if(glob) {
			...
			if(table) {
				...
				for(int i=0; i<idx; ++i) {
					...
					//that value to the pointer. We also add 1 to account for the length
					//character.
...
size_t auto_rating::getStringLength(HINSTANCE instance, UINT id, LANGID lang) {
...
wstring auto_rating::loadString(HINSTANCE instance, UINT id, LANGID lang) {
	const wchar_t* string = lockString(instance, id, lang);
